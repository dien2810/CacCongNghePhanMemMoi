...
function Total() {
    ...
    const shopLocation = JSON.parse(localStorage.getItem('shopLocation')) || { location: { lat: 0, lng: 0 } };
    ...
    const distanceBetween2Point = Math.round(
        distance(userLocation.lat, userLocation.lng, shopLocationData.lat, shopLocationData.lng)
    );

    let totalItemPrice = 0;
    let total = 0;
    const totalShipFee = distanceBetween2Point * 12000;
    ...
    const order = async () => {
        ...
        try {
            ...
            const updatedCart = await clientAPI.service('/cart').find({ username: user.username });
