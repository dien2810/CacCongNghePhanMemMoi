...
const isTokenExpired = (token) => {
  try {
    // Tách lấy phần payload của JWT (ở giữa, phần thứ hai)
    const payloadBase64 = token.split(".")[1];
    // Giải mã payload từ base64 thành JSON
    const payload = JSON.parse(atob(payloadBase64));
    // Kiểm tra thời gian hết hạn (exp)
    return payload.exp < Date.now() / 1000;
  } catch (error) {
    // Nếu có lỗi khi giải mã, trả về false
    return false;
  }
};

export const isAccessTokenExpired = () => {
  return isTokenExpired(getLocalAccessToken());
};

export const isRefreshTokenExpired = () => {
  return isTokenExpired(getLocalRefreshToken());
};
